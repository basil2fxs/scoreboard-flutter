...
class UdpService {
  ...
  ///
  /// For RAMT commands, any already-queued command for the same slot is dropped
  /// so the latest update always wins. This prevents a stale queued command from
  /// overwriting a fresher one (e.g. when style changes while the queue drains).
  void send(String command) {
    ...
    if (command.startsWith('*#1RAMT')) {
      final comma = command.indexOf(',');
      if (comma > 0) {
        final slotPrefix = command.substring(0, comma + 1); // e.g. '*#1RAMT5,'
        _queue.removeWhere((cmd) => cmd.startsWith(slotPrefix));
      }
    }
    _queue.add(command);
  ...
  /// Remove any queued (not yet sent) RAMT commands for the given slot numbers.
  /// Call this before queuing a fresh set of commands for those slots so that
  /// stale commands cannot overwrite the fresh ones.
  void clearSlotsFromQueue(List<int> slots) {
    _queue.removeWhere((cmd) =>
        slots.any((slot) => cmd.startsWith('*#1RAMT$slot,')));
  }
