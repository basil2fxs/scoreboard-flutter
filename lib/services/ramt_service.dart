...
class RamtService {
  ...
  // ─── AFL Quarter ──────────────────────────────────────────────────────────
  void sendAflQuarter(int quarter, DisplayStyle style, {int slot = 8, bool numberOnly = false}) {
    if (quarter == 0) {
      sendRamt(slot, style.color, style.size, style.hAlign, style.vAlign, ' ');
    } else {
      final text = numberOnly ? '$quarter' : 'Q$quarter';
      sendRamt(slot, style.color, style.size, style.hAlign, style.vAlign, text);
    }
  }
