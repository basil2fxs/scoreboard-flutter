    ...
    return SectionCard(
      ...
      child: Column(
        ...
        children: [
          ...
          // Fixed Ad 1–5 rows
          ...(() {
            final selectedCount = List.generate(5, (i) => app.getLaptopAdSelected(i + 1))
                .where((s) => s).length;
            return List.generate(5, (idx) {
              final n = idx + 1;
              final isSelected = app.getLaptopAdSelected(n);
              return _LaptopAdRow(
                adNumber    : n,
                selected    : isSelected,
                duration    : app.getLaptopAdDuration(n),
                currentName : app.getLaptopAdName(n),
                showInfinity: selectedCount == 1 && isSelected,
...
class _LaptopAdRow extends StatefulWidget {
  ...
  const _LaptopAdRow({
    ...
    required this.currentName, required this.showInfinity,
      ...
      child: Column(
        ...
        children: [
          if (ads.isEmpty)
            ...
            const SizedBox(height: 4),
            ...(() {
              final selectedCount = ads.where((ad) => sel[ad.name]?.selected == true).length;
              return ads.asMap().entries.map((entry) {
                ...
                return _AdRow(
                  ...
                  selected    : s.selected,
                  duration    : s.duration,
                  showInfinity: selectedCount == 1 && s.selected,
